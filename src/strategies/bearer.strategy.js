...
module.exports.strategy = new Strategy(function (token, done) {
  ...
  request
    .post('http://192.168.18.29:8080/auth/realms/test/protocol/openid-connect/token/introspect')
    // .post('http://192.168.18.39:8080/realms/keycloak-express/protocol/openid-connect/token/introspect')
    .set('content-type', 'application/x-www-form-urlencoded')
    .send({ token: token, client_id: process.env.CLIENT_ID, client_secret: process.env.CLIENT_SECRET })
    // .send({ token: token, client_id: "abcdefgh" , client_secret: "5jZDL4SvgeFB6boe8a37fZVvenzqpWyB"})
